...
from ai_team.backends.crewai_backend.agents.product_owner import _dict_to_requirements_document
...
def _load_tasks_yaml(config_path: Path | None = None) -> dict[str, Any]:
    ...
    if config_path is not None and config_path.exists():
        ...
        path = Path(_config_pkg.__file__).resolve().parent / "tasks.yaml"
    if not path.exists():
