"""Process-wide spend guard for the LangGraph backend.
...
money. This module tracks cumulative OpenRouter spend for the current run and
aborts once a configurable ceiling is crossed.

The tracker is fed from the httpx response hook in ``langgraph_chat`` (which
already parses every chat-completion body), so it sees real per-call ``cost``
and ``usage`` reported by OpenRouter — no estimation. When the ceiling is
exceeded it raises :class:`BudgetExceededError`, which propagates out of the
graph invoke and fails the run with a clear message instead of continuing to
spend.
