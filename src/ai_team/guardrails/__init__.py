...
class BehavioralGuardrails:
    ...
    @staticmethod
    def validate_scope_control(
        content: str, task_description: str, max_expansion: float = 0.3
    ) -> tuple[bool, str]:
...
class SecurityGuardrails:
    ...
    def validate_no_secrets(cls, content: str) -> tuple[bool, str]:
        ...
        if secrets_found:
            return (
                False,
                f"Secrets detected: {', '.join(set(secrets_found))}. Use environment variables instead.",
            )
    ...
    def validate_iac_security(cls, content: str, iac_type: str = "auto") -> tuple[bool, str]:
        ...
        if iac_type == "auto":
            if (
                "from " in content_lower
                and "run " in content_lower
                and ("copy " in content_lower or "add " in content_lower)
            ):
                detected = "dockerfile"
            elif "version:" in content_lower and (
                "services:" in content_lower or "volumes:" in content_lower
            ):
                detected = "docker_compose"
            elif "apiVersion:" in content_lower and ("kind:" in content_lower):
                detected = "k8s"
            elif "resource " in content_lower and (
                "provider " in content_lower or "terraform " in content_lower
            ):
                detected = "terraform"
        ...
        if detected == "dockerfile":
            ...
            if "sensitive" not in content_lower and "variable " in content_lower:
                violations.append(
                    "Terraform: consider marking sensitive variables with sensitive = true"
                )
            ...
            if (
                re.search(r"effect\s*:\s*allow", content_lower)
                and '"*"' in content
...
class QualityGuardrails:
    ...
    def validate_word_count(
        content: str, min_words: int = 0, max_words: int = 10000
    ) -> tuple[bool, str]:
