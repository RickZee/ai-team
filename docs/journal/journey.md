...
**Why FastAPI + React?** I started with a Rich TUI (still fine over SSH for a single run). But once the system had multiple backends, team profiles, and live streaming from agents, I needed something that could hold WebSocket connections, show a live phase pipeline, and put runs side by side. FastAPI + React with Vite was the pragmatic pick — I already had the API shape from the CLI, and Vite's dev experience is hard to argue with. A Textual TUI (`ai-team-tui`) briefly shared that API; it was later removed — the web dashboard is the supported UI.
