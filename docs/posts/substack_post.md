...
**Be cautious with CrewAI for *autonomous, file-handoff* orchestration.** The Flows abstraction is expressive and the role-based crew model maps intuitively to human team structures — but its runtime fights non-interactive use. Run headless (no TTY), coordinate phases through files on disk, and put it under a wall-clock timeout, and the same trivial calculator task the other two backends finish in **60s (LangGraph) and 200s (Claude SDK)** would *hang indefinitely* for me — and ignore its own timeout, because the live Rich/event-bus threads swallow the kill signal. Getting even the smoke test close to green took two sessions of CrewAI-specific defenses (disabling the Rich console, flattening flow state before serialization to dodge a `RecursionError`, forcing crew memory off to stop an embedder segfault, running pytest *outside* the crew). It's a bad effort-to-reliability ratio for this workload. None of this is the model's fault — the *same* DeepSeek-v3 passes on LangGraph in a minute. If you already live in CrewAI and run it interactively with strong models, it's expressive and pleasant; for unattended file-handoff pipelines, reach for LangGraph or the Claude SDK first.

The bigger lesson cuts two ways. **The model is usually the bottleneck** — DeepSeek-v3 via OpenRouter vs. Claude (SDK) produce structurally similar outputs (same type hints, docstring patterns, edge-case coverage), and the quality gap between model *generations* dwarfs the gap between orchestration frameworks. **But the orchestration runtime can still be the bottleneck for *liveness*** — CrewAI hanging on a task the others breeze through is a framework problem, not a model one. Picking a framework, it turns out, is less about output quality and more about whether its runtime survives being driven by a machine instead of a human.
