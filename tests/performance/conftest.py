...
def _write_benchmark_artifacts(
    ...
    run_real = collector.get("meta", {}).get("run_real", False)
    try:
        sha = subprocess.run(
    ...
    lines: list[str] = [
        "# Performance Report",
        "",
        "Generated by `pytest tests/performance/ -v -s --tb=short`. This file is a "
        "generated artifact — regenerate it, don't hand-edit it.",
        "",
        "## Run metadata",
        "",
    ...
    if not run_real:
        lines.append(
            "- ⚠️ **Mock LLM run — timings are near-zero and meaningless as performance "
            "data.** Set `AI_TEAM_BENCHMARK_FULL=1` for real numbers."
        )
    lines += [
        "",
        "## Per-crew benchmarks",
